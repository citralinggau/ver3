...
BURIQ () {
    curl -sS https://raw.githubusercontent.com/citralinggau/permission/main/ipmini > /root/tmp
...
Name=$(curl -sS https://raw.githubusercontent.com/citralinggau/permission/main/ipmini | grep $MYIP | awk '{print $2}')
...
PERMISSION () {
    MYIP=$(curl -sS ipv4.icanhazip.com)
    IZIN=$(curl -sS https://raw.githubusercontent.com/citralinggau/permission/main/ipmini | awk '{print $4}' | grep $MYIP)
    if [ "$MYIP" = "$IZIN" ]; then
    Bloman
    else
...
Exp=$(curl -sS https://raw.githubusercontent.com/citralinggau/permission/main/ipmini | grep $MYIP | awk '{print $3}')
fi
...
export sem=$( curl -s https://raw.githubusercontent.com/citralinggau/permission/main/versions)
...
echo -e "${BIYellow}□ Server Uptime       = ${RED}$( uptime -p  | cut -d " " -f 2-10000 ) ${NC}"
echo -e "${BIYellow}□ Current Time        = ${RED}$( date -d "0 days" +"%d-%m-%Y | %X" )${NC}"
echo -e "${BIYellow}□ Operating System    = ${RED}$( cat /etc/os-release | grep -w PRETTY_NAME | sed 's/PRETTY_NAME//g' | sed 's/=//g' | sed 's/"//g' ) ( $( uname -m) )${NC}"
echo -e "${BIYellow}□ Current Domain      = ${RED}$( cat /etc/xray/domain )${NC}"
echo -e "${BIYellow}□ Jumlah Ram          = ${RED}${totalram}MB"
...
echo -e "     ${BICyan}[${BIWhite}05${BICyan}] SETING  ${BICyan}[${BIYellow}Menu${BICyan}]${NC}"  "     ${BICyan}[${BIWhite}11${BICyan}] SET REBOOT${BICyan}[${BIYellow}Menu${BICyan}]${NC}" "${BICyan}   │${NC}"
echo -e "     ${BICyan}[${BIWhite}06${BICyan}] UPDATE  ${BICyan}[${BIYellow}Menu${BICyan}]${NC}" "     ${BICyan}[${BIWhite}12${BICyan}] INSTALL UDP${BICyan}[$BIYellow}Menu${BICyan}]${NC}" "${BICyan}  |"
echo -e " ${BICyan}└─────────────────────────────────────────────────────┘${NC}"
echo -e "${BICyan} ┌─────────────────────────────────────────────────────┐${NC}"
