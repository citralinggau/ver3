...
echo -e "${BIYellow}□ Server Uptime       = ${BIRed}$( uptime -p  | cut -d " " -f 2-10000 ) ${NC}"
echo -e "${BIYellow}□ Current Time        = ${BIRed}$( date -d "0 days" +"%d-%m-%Y | %X" )${NC}"
echo -e "${BIYellow}□ Operating System    = ${BIRed}$( cat /etc/os-release | grep -w PRETTY_NAME | sed 's/PRETTY_NAME//g' | sed 's/=//g' | sed 's/"//g' ) ( $( uname -m) )${NC}"
...
echo -e "${BIYellow}□ Expired Script VPS  = ${BIRed}$Exp${NC}"
